...
//definition of huffman class functions
...
void huffman::saveEncodedFile() {
    ...
    while (!tempPQ.empty()) {
        ...
        cout << (char)tempPQ.top()->data << "  " << tempPQ.top()->freq << "   " << tempPQ.top()->code << "   " << endl;;

        in += curr->data;
        //Saving 16 decimal values representing code of curr->data
        s.assign(127 - curr->code.length(), '0');
        s += '1';
        //of << "s-" << s << "  " << curr->code << endl;
        s += curr->code;
        //Saving decimal values of every 8-bit binary code
        //int t = binToDec(s.substr(0, 8));
        //of << s.substr(0, 8) << "   " << t << " " << (char)t << endl;
        int bTD = binToDec(s.substr(0, 8));
        cout << bTD << "-" << (char)bTD << "|";
        in += (char)bTD;
        for (int i = 0; i < 15; i++) {
            s = s.substr(8);
            //t = binToDec(s.substr(0, 8));
            //of << s.substr(0, 8) << "   " << t << " " << (char)t << endl;
            bTD = binToDec(s.substr(0, 8));
            cout << bTD << "-" << (char)bTD << "|";
            in += (char)bTD;
        }
        cout << endl;
